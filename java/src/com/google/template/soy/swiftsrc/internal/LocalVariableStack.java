 ...
 * 
 * TODO class copied from Python stack
 * 
 */
final class LocalVariableStack {

  private final Deque<Map<String, SwiftExpr>> localVarExprs = new ArrayDeque<>();
   ...
   */
  LocalVariableStack addVariable(String name, SwiftExpr varExpression) {
   ...
   */
  @Nullable
  SwiftExpr getVariableExpression(String variableName) {
    for (Map<String, SwiftExpr> frame : localVarExprs) {
      SwiftExpr translation = frame.get(variableName);
      if (translation != null) {
        return translation;
      }
    }
